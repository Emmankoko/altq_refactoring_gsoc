...
int			 npfctl_update_qstats(int, struct pf_altq_node **);
void			 npfctl_insert_altq_node(struct pf_altq_node **,
			    const struct npf_altq, const struct queue_stats);
struct npf_altq_node	*npfctl_find_altq_node(struct pf_altq_node *,
			    const char *, const char *);
void			 npfctl_print_altq_node(int, const struct pf_altq_node *,
			     unsigned, int);
void			 print_cbqstats(struct queue_stats);
void			 print_priqstats(struct queue_stats);
void			 print_hfscstats(struct queue_stats);
void			 npfctl_free_altq_node(struct pf_altq_node *);
...
int
npfctl_show_altq(int dev, int opts)
{
	...
	for (node = root; node != NULL; node = node->next) {
		...
		npfctl_print_altq_node(dev, node, 0, opts);
	}
...
int
npfctl_update_qstats(int dev, struct npf_altq_node **root)
{
...
void
npfctl_print_altq_node(int dev, const struct npf_altq_node *node, unsigned level,
    int opts)
{
...
/*
	...
	if (opts & PF_OPT_VERBOSE)
		npfctl_print_altq_nodestat(dev, node);
...
*/
	for (child = node->children; child != NULL;
	    child = child->next)
		npfctl_print_altq_node(dev, child, level + 1, opts);
}

void
npfctl_print_altq_nodestat(int dev, const struct npf_altq_node *a)
{
	if (a->altq.qid == 0)
		return;
